...
def test_run_qformer_dual_notebook_uses_trajectory_branch_and_dual_config():
    notebook = json.loads(Path("run_qformer_dual.ipynb").read_text(encoding="utf-8"))
    cell0 = "".join(notebook["cells"][0]["source"])
    cell1 = "".join(notebook["cells"][1]["source"])
...
def test_run_qformer_cls_notebook_uses_trajectory_branch_and_cls_config():
    notebook = json.loads(Path("run_qformer_cls.ipynb").read_text(encoding="utf-8"))
    cell0 = "".join(notebook["cells"][0]["source"])
    cell1 = "".join(notebook["cells"][1]["source"])
    train_cell = "".join(notebook["cells"][7]["source"])
    infer_cell = "".join(notebook["cells"][8]["source"])
