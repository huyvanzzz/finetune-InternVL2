...
def resolve_checkpoint_path(checkpoint):
    ...
    if os.path.exists(checkpoint):
        return checkpoint
    print(f"Checkpoint '{checkpoint}' khong phai local path. Dang tai tu HuggingFace...")
...
def parse_args():
    ...
    parser.add_argument("--checkpoint", type=str, default=None, help="Local checkpoint dir hoac HuggingFace Repo ID")
...
def load_internvl_model_and_tokenizer(config):
...
def build_test_dataset(config, args, response_format):
    ...
    print(f"Loaded full test split with {len(dataset_dict['test'])} samples.")

    return WADDatasetForInternVL(
...
def write_prediction_pairs(output_file, checkpoint_label, split_name, detailed_results):
    ...
    print(f"\nDa luu cap GT/Generation tai: {pairs_path}")
    return pairs_path
...
def main():
    ...
    if backend is not None:
        ...
        model, tokenizer = backend.load_model_and_tokenizer(config, checkpoint_dir=args.checkpoint)
        model.img_context_token_id = tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN)
        log_runtime_prompt_state(model, "after_load_before_override")
        model.system_message = SYSTEM_MESSAGE
        log_runtime_prompt_state(model, "after_override")
        ...
        model, tokenizer = load_internvl_model_and_tokenizer(config)
    ...
    if args.checkpoint:
        ...
        if backend is not None:
            ...
            if getattr(model, "qformer_enabled", False):
                print("✓ Backend Q-Former bridge loaded successfully.")
    ...
    test_dataset = build_test_dataset(config, args, response_format)
    ...
    print("\n" + "=" * 50)
    print(f" BAT DAU CHAY DANH GIA TREN TAP: {args.split} (Tong: {len(test_dataset)} samples)")
    print("=" * 50)
    ...
    print(f"\nDa luu chi tiet ket qua tai: {args.output_file}")
